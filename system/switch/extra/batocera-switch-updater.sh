...
version="1.2.81"
wget -q --show-progress --tries=10 --no-check-certificate --no-cache --no-cookies -O "/userdata/system/switch/ryujinx-${version}-linux_x64.tar.gz" "https://foclabroc.freeboxos.fr:55973/share/auyM2KBoFrmv_yBe/ryujinx-1.2.81-linux_x64.tar.gz"
...
mkdir /userdata/system/switch/appimages 2>/dev/null
cd /userdata/system/switch/appimages
citronE="/userdata/system/switch/appimages/citron0.4b.AppImage"
if [ -f "$citronE" ]; then
    ...
    cp /userdata/system/switch/appimages/citron0.4b.AppImage /userdata/system/switch/citron.AppImage 2>/dev/null; fi
        if [ -f "$citronE" ] && [ $(stat -c%s "$citronE") -gt 2048 ]; then
		   ...
		   echo -e "${T}CITRON   [${W}!!${T}] download fail put citron0.4b.AppImage in (/system/switch/appimages) then relaunch script"; fi
