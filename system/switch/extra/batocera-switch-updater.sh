...
version=$(curl -s https://git.ryujinx.app/ryubing/canary/-/releases | grep -oP '/ryubing/canary/-/releases/tag/\K[^"]+' | head -n 1)
wget -q --show-progress --tries=10 --no-check-certificate --no-cache --no-cookies -O "/userdata/system/switch/ryujinx-${version}-linux_x64.tar.gz" "https://git.ryujinx.app/ryubing/canary/-/releases/${version}/downloads/ryujinx-canary-${version}-linux_x64.tar.gz"
link_ryujinx=/userdata/system/switch/ryujinx-${version}-linux_x64.tar.gz
