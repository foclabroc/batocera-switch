...
if (( version == 39 || version == 40 )); then
    echo "[Foclabroc Switch Installer] Batocera version is 39 or 40. Triggering download for version 39/40..."
    sleep 5
    curl -L bit.ly/foclabroc-switchoff-40 | bash
elif (( version == 41 )); then
    echo "[Foclabroc Switch Installer] Batocera version is 41. Triggering download for version 41..."
    sleep 5
    curl -L bit.ly/foclabroc-switchoff | bash
elif (( version == 42 )); then
    echo "[Foclabroc Switch Installer] Batocera version is 42. Triggering download for version 42+..."
    sleep 5
